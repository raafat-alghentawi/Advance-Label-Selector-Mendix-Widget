define([
  ...
  return declare(
    "LabelSelect.widget.LabelSelect",
    [_WidgetBase, _TemplatedMixin],
    {
        ...
        switch (this.searchType) {
          case "datasource":
            ...
            mx.data.get({
              ...
              error: lang.hitch(this, function (err) {
                console.error(
                  this.id +
                    "._fetchCurrentLabels get failed, err: " +
                    err.toString()
                );
                this._executeCallback(
                  callback,
                  "_fetchCurrentLabels data err cb"
                );
              }),
            });
            break;
          case "microflow":
            ...
            for (let i = 0; i < items.length; i++) {
              ...
              if (domClass.contains(element, "tagit-new")) {
                enteredValue = items[j].innerText;
                break;
              }
              j=i;
            }

            this._contextObj.set(this.searchStringAttribute, enteredValue);

            this._execMfSearch(
              this._contextObj.getGuid(),
              self.searchMicroflow,
              lang.hitch(this, this._processTags, callback)
            );
            break;
        }
      },
      ...
      _renderCurrentTags: function (currentTags, callback) {
        ...
        dojoArray.forEach(
          ...
          function (tagObj, index) {
            ...
            color = this._colorAttribute
              ? dom.escapeString(tagObj.get(this._colorAttribute))
              : null;
      ...
      _execMfSearch: function (guid, mf, cb, showProgress, message) {
        var self = this;
        if (guid && mf) {
          var options = {
            params: {
      ...
      _saveObject: function () {
        ...
        var method =
          !mx.version || (mx.version && parseInt(mx.version.split(".")[0]) < 7)
            ? "save"
            : "commit";
        mx.data[method](
          {
      ...
      _setOptions: function (tagArray) {
        ...
        var options = {
          ...
          afterTagRemoved: lang.hitch(this, function (event, ui) {
            ...
            var tagObj = this._tagCache[ui.tagLabel];
            if (tagObj) {
              this._contextObj.removeReferences(this._refAttribute, [
                tagObj.getGuid(),
              ]);
              this._saveObject();
            } else {
